...
header = <<~HEADER_CODE
  eval(a=%(->z{n=++0;$h='eval(a=%('+a;$z=('))[$z=+%`'+$z).lines;eval(eval((?;*9+z+?;*2).scan(/^.{9}|.{9}$/)*'')*'')}
  ))[$z=+%`
HEADER_CODE
...
eval_code = "$z.map{_1[21,72].delete(32.chr)}"
side_code_chars = [rs_code, expand_code, eval_code].join(';').chars
zwas = innercode.map{'▄'*9+_1+'▄'*9}
zwas[0]=header.lines[1].strip+zwas[0][9..]
zwas[-1][-2,2] = '`]'
